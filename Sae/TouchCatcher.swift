...
struct TouchCatcher: UIViewRepresentable {
    /// 첫 접촉(터치 다운) 한 건을 전달한다. 릴리즈가 아니라 다운 순간이 응답이다(§4).
    var onTouchDown: (TouchSample) -> Void
...
/// 저수준에서 잡은 터치 다운 한 건 — 응답 시각과, 그 시각이 어느 시계에서 왔는지 확인할 기준값.
struct TouchSample {
    /// 응답 시각 = `UITouch.timestamp`(터치의 하드웨어 이벤트 시각, 단조 기준). 반응시간의 뺄셈 항.
    let timestamp: TimeInterval
    /// `timestamp`를 읽은 **직후** 읽은 공통 기준 `CACurrentMediaTime()`. 시계 계약 확인용(§8-2).
...
final class TouchCatchingView: UIView {
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        // 측정값(`touch.timestamp`)을 먼저 읽고, 그다음 기준 시각을 읽는다. 순서가 중요하다 —
        // 기준을 먼저 읽으면 두 값 사이에 이 핸들러의 실행 지연이 끼어 계약 확인이 흐려진다.
        // 이 기준값 읽기는 응답 시각을 얻은 뒤라 측정 자체를 오염시키지 않는다(제1조 1항).
        let sample = TouchSample(timestamp: touch.timestamp, referenceTime: CACurrentMediaTime())
        onTouchDown?(sample)
    }
}
