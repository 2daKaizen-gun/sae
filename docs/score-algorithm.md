...
- **최종 수정:** 2026-09-26 (타당도 게이트 서술을 구현과 일치)
- **버전:** v0 (기본값·앵커는 **튜닝 대상**. → [열린 결정](#열린-결정))
- **상태:** 구현 중 (Phase 2, 이슈 #4)

---

## Phase 2 확정 사항 (2026-09-23)

구현에 들어가며 아래 세 가지를 확정한다. 나머지는 여전히 [열린 결정](#열린-결정)이다.

**① MVP의 冴え度는 PVT 단독으로 산출한다.**
HRV·손떨림을 점수에 넣지 않는다. 이 문서 스스로 HRV 절대 매핑을 "거친 근사"(§2)라 하고, 손떨림 앵커는 아직 **값이 없다**(§3). 근거가 약한 신호를 대표 숫자에 섞으면 이 제품의 차별점(정직성)을 가장 약한 증거에 거는 셈이 된다(제2조 1항). 두 신호는 Phase 3에서 **측정·표시까지** 하고, 점수 편입은 앵커를 실측한 뒤에 다시 판단한다. 따라서 §4의 재정규화는 MVP에서 **항상 각성 단독(가중치 1.00)** 으로 귀결된다.

**② 세션 파라미터: 12 trial · 응답 타임아웃 10초 · ISI 2~10초.**
trial 수를 고정하는 이유는 **lapse율의 분모를 세션마다 같게** 두기 위해서다(§1-1). 시간 고정(90초)으로 하면 trial 수가 세션마다 달라져 날짜 간 비교가 흔들린다. 예상 소요는 ISI 평균 6초 + 응답 ≈ **80초 안팎**으로 `CONCEPT.md`의 "90초" 취지와 맞는다.

**③ 타임아웃 10초는 UX 판단이지 문헌값이 아니다.**
직전 값 30초는 무응답 1건에 30초를 쓰게 해 90초 세션과 맞지 않았다. 다만 **PVT 표준의 정확한 타임아웃 관례는 확인하지 못했다** — 그러므로 10초를 "표준"이라 부르지 않는다(제2조 1항). 원자료(`PVTTrial`의 시각)를 보존하므로 다른 문턱으로 **재계산할 수 있고**, 문헌을 확인하면 값을 고치고 이 줄을 갱신한다.
...
다음이면 `PVTSession.isValid = false` → **그날 `DailyScore`를 만들지 않고**, 화면에는 점수 대신 무효 사유를 보인다. 억지 점수보다 "오늘은 못 쟀어요"가 정직하다.

- false start ≥ 3, 또는 응답(유효+lapse) < 최소치(5). **무응답 과다는 별도 규칙이 아니라 이 최소 응답 규칙으로 걸린다** — 12 trial 중 무응답이 8개 이상이면 응답이 5 미만이 된다.
- 임계값 자체는 [열린 결정](#열린-결정).
...
- **타당도 게이트 임계**(false start 3, 최소 응답 5) — 실측 후 확정(§1-3). trial 수가 12로 고정됐으므로 **최소 응답 5(=42%)는 느슨한 편**이다. 다만 어느 값이 맞는지는 실데이터 없이 정할 수 없어, 근거 없이 올리는 대신 현 값을 유지하고 열린 결정으로 남긴다(제2조 1항).
