...
- **최종 수정:** 2026-09-26 (Phase 2 기록 보강 · 이슈 #4 마감 · 문서·코드 정합)
...
- **코드 정리(브랜치 `code-doc-drift-cleanup`)** — 전 소스를 읽고 문서와 어긋나거나 불필요한 것을 고침:
  - **ISI 시드를 세션마다 새로 뽑음**(`b6d56c2`) — 기본 시드가 상수라 **매 세션 같은 12개 간격이 같은 순서로** 나왔다. 리듬을 외우면 자극을 예측할 수 있어 PVT가 무너진다(제1조). 실제 간격은 `interStimulusMs`로 저장되므로 재현성은 유지
  - **첫 화면의 "SaeTiming 링크 증명용" lapse 문턱 표시 제거** + 해당 카탈로그 키 삭제(`496c14c`) — 랩이 패키지로 세션·점수를 돌리므로 더는 증명할 게 없음
  - **낡은 코드 주석 정정**(`8ed3b04`) — 없어진 `DailyScore.explanation` 참조, 무효 세션 처리 애매한 서술, 패키지 헤더(점수 계산도 담음)
  - **설계 문서를 구현에 맞춤**(`ef06ef4`) — data-model "무응답이면 `isLapse=true`" → 실제는 false(무응답≠lapse), `DailyScore.session` 관계·날짜 조회 확정, score-algorithm 게이트의 "무응답 과다" = 최소 응답 규칙으로 걸림
  - 로컬 `build/`(7월 DerivedData 104MB, gitignore 대상) 삭제
  - **검증:** `swift test` 70개 그린, 앱 빌드 성공. 시뮬에서 무탭 세션 2회 → 저장된 ISI 수열이 서로 다르고 옛 고정 수열(5847/6887/2066/…)과도 다름을 SQLite로 확인. 두 세션 모두 `tooFewValidTrials`로 무효·`DailyScore` 미생성(설계대로)

### 🔄 진행 중 (미완)
- 없음. 코드 정리는 **PR #7로 `main`에 머지**(`5e7006f`), 브랜치 삭제. 원격 브랜치는 `main`뿐
...
- PR: **#7 MERGED**(코드 정리) / 이슈: **#4 CLOSED**(2026-09-26) · #5 OPEN
